...
const dailyDeals = [
  {
    ...
    description: 'Premium organic tomatoes sourced directly from local Nigerian farms. These vine-ripened tomatoes are carefully selected for their superior taste and quality. Perfect for salads, cooking, or making fresh sauce.',
  ...
  return (
    <View className="flex-1 bg-['#f8f8f8'] px-4 py-2">
      ...
      <View className='flex-row items-center mb-4 mt-2'>
        ...
        <Text className="text-black-600 font-semibold text-lg" style={{fontSize: 18}}>Deliver to: <Text className="font-bold text-black">123 Victoria Island, Lagos</Text></Text>
      </View>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mt-2">
          {['All', 'Vegetables', 'Fruits', 'Meat'].map((category, index) => (
            <TouchableOpacity
              ...
              onPress={() => router.push({ pathname: '/ProductList', params: { category } })}
            >
              <Text className="font-bold text-sm" style={{color: '#058044'}}>{category}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
          ...
          {dailyDeals.map((item, index) => (
            ...
            >
              ...
              <View className="flex-row justify-between items-center mt-4">
                <View>
              <Text className="text-lg font-bold" style={{ color: '#058044' }}>
                ₦{parseFloat(item.price).toFixed(2)}
              </Text>
                  {item.oldPrice && (
                    <Text className="text-gray-500 line-through">
                      ₦{parseFloat(item.oldPrice).toFixed(2)}  {/* Convert to number */}
                    </Text>
                  )}
                </View>
                <TouchableOpacity
        ...
        <View className="flex-row justify-between mb-4">
          {['Vegetables', 'Fruits', 'Dairy', 'Meat'].map((category, index) => (
            ...
            >
              ...
              <Text className="text-gray-500 text-sm font-semibold">{category}</Text>
            </TouchableOpacity>
          ))}
        </View>
